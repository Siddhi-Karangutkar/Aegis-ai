...
def get_local_model():
    ...
    model_path = os.path.join(os.path.dirname(__file__), "..", "ml", "email_model.pkl")
    if os.path.exists(model_path):
        try:
            with open(model_path, 'rb') as f:
                import joblib
                _local_model = joblib.load(f)
                return _local_model
        except: pass
    return None
...
def analyze_phishing_advanced(text: str) -> dict:
    ...
    hf_prob, hf_conf = 0.0, 0.0

    # ── Composite Risk Calculation ──
    if local_prob is not None:
        # Local model is primary
        threat_score = int((local_prob * 60) + (pattern_score * 0.3) + (nlp_score * 0.1))
        # For logistic regression, confidence is how far the prob is from 0.5
        confidence = max(local_prob, 1.0 - local_prob)
        ai_summary_prefix = "Local AI model and scam pattern analysis identifies: "
        engine_source = "Local ML + Rule Engine"
        ml_probability = local_prob
    else:
        # Fallback to HuggingFace
        hf_prob, hf_conf = get_hf_prediction(text)
        threat_score = int((hf_prob * 60) + (pattern_score * 0.3) + (nlp_score * 0.1))
        confidence = hf_conf
        ai_summary_prefix = "HuggingFace BERT model and scam pattern analysis identifies: "
        engine_source = "HuggingFace API Engine"
        ml_probability = hf_prob
